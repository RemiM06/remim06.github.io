...
// URL des données GeoJSON de Nice Côte d'Azur
...
// Chargement et affichage des données GeoJSON
fetch(geojsonUrl)
    .then(response => {
        if (!response.ok) {
...
// Chargement et affichage de la route
fetch(routeUrl)
    .then(response => {
        if (!response.ok) {
            throw new Error(`Erreur HTTP : ${response.status}`);
